...
const { PAYLOAD_SECRET, MONGODB_URL } = process.env
            ...
            if (versionDocs.length === 0) {
              try {
                ...
                payload.logger.error(
                  `Unable to create version corresponding with collection ${slug} document ID ${
                    doc.id as string
                  }`,
                  e?.errors ?? e
                )
              ...
              payload.logger.info(
                `Created version corresponding with ${slug} document ID ${doc.id as string}`
              )
            }
          })
        )
